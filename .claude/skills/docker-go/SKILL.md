...
Целевая версия: Go 1.27, ниже не поддерживается (политика версий — скилл `go-quality`). Compose — мажор v5.
...
Плюс `.dockerignore` — в корне контекста сборки, не в `docker/`. Контекст здесь — корень
репозитория, тот же каталог, где лежит `go.mod` (раскладка — скилл `go-project-structure`).
Если проект уже живёт по старой раскладке с `app/go.mod` и `context: ../app`, это не повод
его переносить: правила ниже применяй к его контексту.
...
Пути в этом файле читаются в двух разных системах координат, и их нельзя путать:

- `COPY go.mod go.sum ./` и `./cmd/app` — от **корня контекста**, то есть от корня
  репозитория. Голые имена работают именно потому, что `go.mod` лежит там же.
- `WORKDIR /app`, `/go/bin/app`, `/app/migrations` — пути **внутри образа**. К раскладке
  репозитория они отношения не имеют и не меняются, даже если каталоги в репозитории
  переедут. `COPY --from=builder /app/migrations` тянет то, что положил `COPY . .` в builder.
...
Проверено на 2026-09-15: у `golang` есть только `-alpine3.23` и `-alpine3.24`, свежая серия
самого `alpine` — 3.24. Утверждение про доступные теги стареет быстрее остального скилла,
поэтому перед сменой версии проверяй его сам, а не верь строчкам выше:

```bash
curl -s "https://hub.docker.com/v2/repositories/library/golang/tags?page_size=100&name=alpine" \
  | grep -o '"name":"[^"]*"'
```
...
Ловушка ровно одна: **выставленная переменная `GOMAXPROCS` отключает и учёт cgroup-лимита, и
периодическое перечитывание** — то же делает вызов `runtime.GOMAXPROCS`. То есть `environment:
GOMAXPROCS: "4"` в compose не «помогает рантайму», а глушит всю автоматику и прибивает
значение намертво, мимо лимита контейнера. Не задавать её, если нет причины важнее.
...
с реальными паролями — он ляжет в слой. Файл кладётся в корень контекста, то есть рядом
с `go.mod`:
...
Первые шесть строк — следствие того, что контекст = корень репозитория: инфраструктурные
каталоги лежат внутри него и без исключения уедут демону. Дело тут не только в размере
контекста, а в кеше: слой `COPY . .` инвалидируется правкой **любого** файла в контексте,
поэтому без этих строк редактирование `compose.production.yaml` или спеки в `api/` гонит
пересборку всего модуля. `api/` исключается смело — в образ едет сгенерированный по спеке
код, а не сама спека.

Список — deny-list, и у него есть слабое место: новый каталог в корне по умолчанию попадает
в контекст, пока кто-нибудь не вспомнит про `.dockerignore`. Заводишь каталог в корне —
сразу решай, нужен ли он сборке.

### -buildvcs=false: зачем на самом деле

Дефолт — `auto`. Если `.git` в контекст не попал — а он не попадает, потому что исключён
в `.dockerignore` выше, — сборка проходит **успешно**, просто не проставляет VCS-штамп в
...
`COPY` в образе нет намеренно: исходники монтируются на запуске. Спека, конфиг генератора
и каталог для результата лежат в одном дереве, поэтому хватает одного маунта корня —
из корня репозитория:

```bash
docker run --rm -v "$PWD":/app -w /app codegen \
  -config docker/images/codegen/oapi-config/bonds.yaml api/bonds.yaml
```
...
### Как Compose резолвит пути в build

Два разных отсчёта, и их постоянно путают:

- `context` считается от **project directory** — каталога первого `-f`-файла. Компоуз лежит
  в `docker/`, значит `..` — это корень репозитория.
- `dockerfile` считается **от контекста**, а не от compose-файла. Поэтому путь пишется от
  корня репозитория и не начинается с `../`.

Отсюда и пара `context: ..` + `dockerfile: docker/images/app/Dockerfile.production`. То же
самое руками, из корня репозитория:

```bash
docker build -f docker/images/app/Dockerfile.production -t project:dev .
```
...
Тег пиннут до alpine-базы (`18-alpine3.24`), а не оставлен как `18-alpine`, по той же причине,
по которой в builder не берётся `golang:latest`: плавающий суффикс однажды переедет на новую
серию alpine и поменяет окружение под базой, хотя мажор Postgres остался тем же. Для dev-базы
цена ошибки невелика, но воспроизводимость здесь бесплатна.
